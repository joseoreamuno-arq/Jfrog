...
Create step s_delta_relorder AS (
INSERT INTO Delta(TableName='delta.relorder',Overwrite=true,Unlogged=true)
    ...
    FROM  delta.pstg_order_item_final pstg
    JOIN xc_co_item_asgnmt xcoia ON pstg.Related_Order_Code = xcoia.Order_Code AND Contains(xcoia.Item_Code,'Churn')=true
)
/*****/
...
-- Update the Cloud_Alliance_Partner logic to also use the Cloud_Alliance_Partner_Multi field from SFDC 
-- Update the Partner Manager logic to use the Channel_Partner_Account_Owner field insead of the title 

Alter step s_load_staging_order_item_assignment AS (
Insert Into staging.order_item_assignment (order_code, item_code, Employee_ID, split_amount_pct)
    ...
    WHERE 1=1
    AND err.created_timestamp IS null
    AND (pstg.Cloud_Alliance_Partner_Multi INN ('Azure', 'GCP') OR pstg.Cloud_Alliance_Partner IN ('Azure', 'GCP'))
    AND pstg.BATCH_TYPE = 'Opportunity'
