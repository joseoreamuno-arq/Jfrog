Manager Incentive Details Export Kickoff
p_incentive_details_export_manager_kickoff
...
Set v_Reporting_Period_Names *= Select Nvl(:Reporting_Period_Names,:v_period_name) FROM Empty();
...
Create step s_set_running_pipeline_incentive_details_export_manager_kickoff AS (
SET v_running_pipeline *= 'p_incentive_details_export_manager_kickoff'
)
...
SELECT IF :v_incentive_details_eid IS NOT NULL THEN true ELSE false END FROM Empty()
SELECT IF Count(C1)=0 THEN false ELSE true END  FROM delta.user_list

Create step s_queue_incentive_details_export_manager AS (
call QueueIncentProcessGroup(Input=(select 'Manager Incentive Details Export Process' process_group_name, :v_period_name period_name, '[{"Manager Incentive Details Export Preprocess":[{ "Email_Distribution_List": "'||:v_email_to||'"},{ "Rep_Id": "'||:Rep_Id||'"},{ "Territory": "'||:Territory||'"},{ "Reporting_Period_Names": "'||:Reporting_Period_Names||'"},{ "Distribute": "'||:v_distibution_flag||'"}]}]' parameter_overrides));
)
