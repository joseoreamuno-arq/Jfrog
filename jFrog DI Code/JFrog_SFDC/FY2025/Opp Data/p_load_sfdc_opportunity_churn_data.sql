...
INSERT into Delta(TableName='delta.pstg_order_item_churn_prestage', Overwrite=true, Unlogged=true)
    ...
    , CASE WHEN (Opportunity.Solution_Engineer__c IS NOT NULL) THEN usr.Name ELSE NULL END AS Opp_Strategic_Opp_Solution_Architect_name
    ...
    from Opportunity where Opportunity.Churn_Date__c >= '||:v_var_sales_data_pp_start_date ||'
    AND Opportunity.Churn_Date__c <= '||:v_var_sales_data_pp_end_date ||' 
    AND (Opportunity.StageName not in(''06-Closed Won'',''06-Closed Lost'') 
    OR Opportunity.CloseDate >= ' || :v_var_sales_data_pp_end_date || ')',
