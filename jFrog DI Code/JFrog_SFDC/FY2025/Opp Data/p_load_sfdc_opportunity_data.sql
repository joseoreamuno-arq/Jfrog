...
INSERT INTO Delta(TableName='delta.pstg_order_item_final',Overwrite=true,Unlogged=true)
    ...
    , account.Business_Unit__r.Pod__c, account.Business_Unit__r.Tier_Level__c, account.Business_Unit__r.Tier_Manager__r.Name, account.Business_Unit__r.Tier_Manager__c, account.Business_Unit__r.Regional_VP__r.Name, account.Business_Unit__r.Pod_Manager__r.Name, CSM__c, Opportunity.Type, Opportunity.Owner.Name,
    ...
    from Opportunity
    where Opportunity.StageName in(''06-Closed Won'',''06-Closed Lost'')
    and CloseDate >= ' || :v_var_sales_data_pp_start_date || ' and CloseDate <= ' || :v_var_sales_data_pp_end_date,
    UserName=:v_sfdc_user, Password=:v_sfdc_passwd, Environment=:v_sfdc_env, ReadAll=false, Retries=2, RetryInterval=1 ) opp
    LEFT JOIN delta.generic_user_load usr on LeftSide(opp.Solution_Engineer__c,15) = usr.Id
    LEFT JOIN (SELECT order_code,item_code,incentive_date FROM xactly.xc_comp_order_item coi
    JOIN (SELECT Max(Incentive_Date) AS LatestDate,
    Order_Code FROM xactly.xc_comp_order_item
    WHERE Incentive_Date < :v_var_sales_data_pp_start_date AND Churn_Date IS NOT NULL GROUP BY Order_Code) LatestOrders
    ON coi.order_code = LatestOrders.order_code AND coi.incentive_date = LatestOrders.LatestDate
    ) relorders on(relorders.order_code = opp.Id)
    ...
    WHERE 1=1
    AND (Approval_Process__r.Description__c NOT INN ('SDR Approval Process', '"SDR Approval Process"'))
...
sfdc_account_security_seat_count_agg
INSERT INTO Delta(TableName='delta.sfdc_account_security_seat_count_agg',Overwrite=true,Unlogged=true)
    SELECT
    Year(Opportunity.CloseDate) AS Close_Year
    , SBQQ__Account__r.Name AS Account_Name
    , SBQQ__Quote__r.SBQQ__Account__c AS Account_Id
    , SBQQ__QuoteLine__r.Unit_Measre__c
    , SUM(SBQQ__QuoteLine__r.GB__c) AS Security_Seat_Count_Agg
    ...
    , Opportunity.CloseDate
    , SBQQ__QuoteLine__c
    , SBQQ__QuoteLine__r.SBQQ__Quote__r.SBQQ__Account__r.Name
    , SBQQ__QuoteLine__r.SBQQ__Quote__r.SBQQ__Account__c
    , SBQQ__QuoteLine__r.Unit_Measre__c
    , SBQQ__QuoteLine__r.GB__c
    FROM OpportunityLineItem
    WHERE SBQQ__QuoteLine__r.Unit_Measre__c = ''Security Seats''
    AND Opportunity.StageName in(''06-Closed Won'',''06-Closed Lost'') and Opportunity.CloseDate >= ' || ToDate(StartOfYear(SubtractTimeInterval(:v_var_sales_data_pp_start_date,3,'Years'))) || ' AND Opportunity.CloseDate <= ' || :v_var_sales_data_pp_end_date
...
s_load_opp_clean_pstg_order_item_dump
...
LEFT JOIN (SELECT Order_Code, Item_Code, System_Approval_Description, Max(Order_Date) AS Order_Date, Max(Deal_Date) AS Deal_Date, Max(Approval_Process_Created_Date) AS Approval_Process_Created_Date FROM delta.pstg_order_item_final) b ON a.order_code = b.order_code AND a.item_code = b.item_code AND System_Approval_Description = 'New Business'
LEFT JOIN delta.sfdc_account_security_seat_count_agg sc ON a.Account_Id = sc.Account_Id AND Year(a.Incentive_Date)=sc.Close_Year
LEFT JOIN delta.sfdc_account_security_seat_count_agg sc1 ON a.Account_Id = sc1.Account_Id AND Year(SubtractTimeInterval(a.Incentive_Date,1,'Years')) =sc1.Close_Year
LEFT JOIN delta.sfdc_account_security_seat_count_agg sc2 ON a.Account_Id = sc2.Account_Id AND Year(SubtractTimeInterval(a.Incentive_Date,2,'Years'))=sc2.Close_Year
LEFT JOIN delta.sfdc_account_security_seat_count_agg sc3 ON a.Account_Id = sc3.Account_Id AND Year(SubtractTimeInterval(a.Incentive_Date,3,'Years'))=sc3.Close_Year
WHERE 1=1
-- AND (a.System_Approval_Description IS NOT NULL AND ToDecimal(a.Final_ARR_Growth) <> 0.00 AND a.Stagename INN ('06-Closed Won', '06-Closed Lost'))
-- OR (a.System_Approval_Description != 'Expiring ARR' AND a.Churn_Date >= :v_var_sales_data_pp_start_date AND a.Stagename ='06-Closed Lost')
;
s_delete_opp_expiring_arr_pstg_order_item
